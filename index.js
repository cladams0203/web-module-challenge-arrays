...
function addFlavor(arry, flavor) {
  arry.unshift(flavor);
  return arry;
}
...
function removeFlavorByName(arry, flavorToRemove) {
  return arry.splice(arry.indexOf(flavorToRemove), 1);
}
...
function copy(arry, copyArray) {
  return (copyArray = [...arry]);
}
const newArry = [];
console.log(copy(originalFlavors, newArry));
...
function filterByWord(arry, flavor) {
  const newArry = [];
  for (let i = 0; i < arry.length; i++) {
    if (arry[i].includes(flavor)) {
      newArry.push(arry[i]);
    }
  }
  return newArry;
